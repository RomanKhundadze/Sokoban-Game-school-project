...
WINDOW *Gameboard::getWindowHandle() { return wnd; }

WINDOW *Gameboard::getStatusWindowHandle() { return stw; }
    ...
    if (f.good()) {
        while (!f.eof()) {
            ...
            if (std::string::npos == str.find(';')) {
                vec.push_back(str);
                for (std::string xachse : vec) {
                    if (xachse.length() > max.x) {
                        max.x = xachse.length();
                    }
                }
            }
        ...
        std::cout << "Map Existiert nicht!" << std::endl;
        endwin();
        return 0;
    }
}
...
Point Gameboard::getPlayer() { return player; }
